module Regexp =
    ...
    let rec matchLengths (pattern: Pattern) (s: string) : seq<int> =
        ...
        | Alter (lhs, rhs) ->
            seq {
                for n in matchLengths lhs s -> n
                for n in matchLengths rhs s -> n
            }

    // s の先頭から始まる部分文字列の中で pattern にマッチするものを長い順に返す
    let matchSubstrings (pattern: Pattern) (s: string) : seq<string> =
        matchLengths pattern s |> Seq.map (fun len -> s.Substring(0, len))
    ...
    let p = compile "<(world|h.*)>"
    printfn "%A" p
    printfn "%A" (matchSubstrings p "<hello>world</hello>") //=> seq ["<hello>world</hello>"; "<hello>"]
